...
void main() {
  ...
  group('exact service words', () {
    test(
      'single word "Care" finds Caregiver Visit',
    ...
    test(
      'direct name',
      () => expect(top('coconut plucking'), 'Coconut Plucking'),
    );
  });
  ...
  group('natural sentences without the service word', () {
    test(
      'leaking tap',
      () => expect(top('the tap in my bathroom is leaking'), 'Plumbing'),
    );
    test(
      'dripping water',
      () => expect(top('water is dripping from the pipe'), 'Plumbing'),
    );
    test(
      'broken fan',
      () => expect(top('my fan is not working'), 'Electrical'),
    );
    test(
      'no power',
      () => expect(top('there is no power in the wall socket'), 'Electrical'),
    );
    test(
      'overgrown grass',
      () => expect(top('the grass has grown too much'), 'Gardening'),
    );
    test(
      'care for grandmother',
      () => expect(
        top('someone to look after my grandmother'),
        'Caregiver Visit',
      ),
    );
    test(
      'buy pills',
      () => expect(top('need to buy pills for my father'), 'Medicine Delivery'),
    );
  });
  ...
  group('natural Malayalam', () {
    test(
      'leaking pipe',
      () => expect(top('വീട്ടിലെ പൈപ്പ് ചോരുന്നു'), 'Plumbing'),
    );
    test(
      'coconut plucking',
      () => expect(top('തേങ്ങ പറിക്കാൻ ആളെ വേണം'), 'Coconut Plucking'),
    );
    test('no current', () => expect(top('കറന്റ് പോയി'), 'Electrical'));
    test(
      'medicines',
      () => expect(top('അമ്മയ്ക്ക് മരുന്ന് വേണം'), 'Medicine Delivery'),
    );
  });
  ...
  group('natural Tamil', () {
    ...
    test(
      'care for grandmother',
      () => expect(top('பாட்டியை பார்த்துக்க ஆள் வேணும்'), 'Caregiver Visit'),
    );
    test(
      'tablets',
      () => expect(top('மாத்திரை வாங்கணும்'), 'Medicine Delivery'),
    );
    test(
      'coconut inflected',
      () => expect(top('தேங்காயை பறிக்கணும்'), 'Coconut Plucking'),
    );
  });
  ...
  group('natural Hindi', () {
    ...
    test(
      'house cleaning',
      () => expect(top('घर की सफाई करनी है'), 'House Cleaning'),
    );
    test(
      'nurse for grandmother',
      () => expect(top('दादी के लिए नर्स चाहिए'), 'Caregiver Visit'),
    );
  ...
  group('fuzzy tolerance', () {
    test(
      'plural pipes matches pipe',
      () => expect(top('the pipes are very old'), 'Plumbing'),
    );
    test(
      'gardening stem',
      () => expect(top('need some gardening done'), 'Gardening'),
    );
  ...
  group('no false positives', () {
    test('unrelated speech', () {
      expect(
        matchServicesToTranscript('what is the weather today', pool),
        isEmpty,
      );
    });
